...
@section('content')
    <div class="col-lg-9 col-12">
        <div class="blog-page">
            ...
            {!! Form::model($post ,['route'=>['frontend.dashboard.update' , $post->id] , 'method'=>'put' ,'files'=>true]) !!}
        ...
        $(function(){
           ...
           $('#post-images').fileinput({
               ...
               initialPreviewConfig: [
                   @if($post->media->count() > 0 )
                   @foreach($post->media as $media)
                    {caption:"{{ $media->file_name }}", size:{{$media->file_size}},width:"120px", url:"{{route('frontend.dashboard.media.destroy',[$media->id,'_token'=>csrf_token()])}}", key:"{{$media->id}}"},
                   @endforeach
                   @endif
               ],
           })
